...
serve(async (req) => {
  ...
  try {
    ...
    if (!ALPHA_VANTAGE_API_KEY) {
      console.log('ALPHA_VANTAGE_API_KEY environment variable is not set, using mock data');
    } else {
    ...
    // If no API key is available, immediately use mock data for all actions
    if (!ALPHA_VANTAGE_API_KEY) {
      return handleMockData(action, params, corsHeaders);
    }
    ...
    switch (action) {
      case 'search':
        ...
        try {
          ...
          console.error('Error in stock search, using mock data:', searchError);
      ...
      case 'quote':
        ...
        try {
          console.log(`Fetching quote data from: ${apiUrl}`);
          const response = await fetch(apiUrl);
          ...
          console.error('Error in stock quote, using mock data:', quoteError);
      ...
      case 'daily':
        ...
        try {
          console.log(`Fetching daily data from: ${apiUrl}`);
          const response = await fetch(apiUrl);
          ...
          console.error('Error in daily data, using mock data:', dailyError);
