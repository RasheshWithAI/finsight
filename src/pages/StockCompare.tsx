...
import { ArrowLeft, Search, X, Plus, PlusCircle } from "lucide-react";
...
const StockCompare = () => {
  ...
  useEffect(() => {
    if (searchTerm) {
      const results = mockStocks.filter(stock => !stocks.some(s => s.id === stock.id) && (stock.name.toLowerCase().includes(searchTerm.toLowerCase()) || stock.symbol.toLowerCase().includes(searchTerm.toLowerCase())));
  ...
  return <div className="container px-4 py-6 animate-fade-in">
      ...
      <Card className="financial-card mb-6 bg-amber-500">
        <CardContent className="p-4">
          <div className="mb-4">
            ...
            {stocks.length === 0 ? <p className="text-aura-secondary-text py-2 text-gray-50 mx-[2px] my-0">
                Add stocks to compare using the search below
              </p> : <div className="flex flex-wrap gap-2 mb-3">
                {stocks.map(stock => <div key={stock.id} className="flex items-center bg-aura-dark-gray px-3 py-1 rounded-full">
            ...
            {stocks.length < 3 && <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                <Input placeholder="Search for stocks to compare..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-neutral-950 rounded-2xl py-0 my-0 mx-0 px-[41px]" />
                
                {showResults && searchResults.length > 0 && <div className="absolute z-10 mt-1 w-full bg-aura-dark-gray border border-gray-800 rounded-md shadow-lg max-h-60 overflow-y-auto">
                    {searchResults.map(stock => <div key={stock.id} className="px-4 py-2 hover:bg-aura-charcoal cursor-pointer flex justify-between items-center" onClick={() => handleAddStock(stock)}>
                        <div>
                ...
                {showResults && searchResults.length === 0 && searchTerm && <div className="absolute z-10 mt-1 w-full bg-aura-dark-gray border border-gray-800 rounded-md shadow-lg p-3 text-center">
                    <p className="text-aura-secondary-text">No matching stocks found</p>
                  </div>}
              </div>}
            
            {stocks.length === 0 && <div className="mt-3 flex justify-center">
                <Button variant="outline" onClick={() => handleAddStock(mockStocks[0])} className="flex items-center bg-blue-950 hover:bg-blue-800 rounded-2xl">
      ...
      {stocks.length > 0 && <Card className="financial-card mb-6">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg">Performance Comparison</CardTitle>
          </CardHeader>
          <CardContent className="p-4">
            <StockLineChart symbol={stocks.map(s => s.symbol).join(" vs. ")} showOverlay={stocks.length > 1} comparisonData={stocks.length > 1 ? comparisonData : undefined} />
          </CardContent>
        </Card>}

      {/* Side by Side Comparison */}
      {stocks.length > 1 && <Card className="financial-card">
          <CardHeader className="pb-2">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  {stocks.map(stock => <th key={stock.id} className="p-2 text-left text-xs font-medium">
              ...
              <tbody>
                <tr className="border-b border-gray-800">
                  ...
                  {stocks.map(stock => <td key={stock.id} className="p-2 text-left font-medium">
