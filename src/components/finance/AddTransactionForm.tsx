...
const AddTransactionForm = ({
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-gray-800 text-white sm:max-w-[500px] rounded-2xl">
        <DialogHeader>
            ...
            <div className="col-span-3">
              <Popover>
                ...
                <PopoverContent className="w-auto p-0 bg-gray-800 border-gray-700">
                  <Calendar mode="single" selected={date} onSelect={date => date && setDate(date)} initialFocus className="p-3 pointer-events-auto" />
                </PopoverContent>
              </Popover>
            </div>
          </div>
                ...
                <SelectContent className="bg-gray-700 border-gray-600">
                  <div className="sticky top-0 p-2 bg-gray-700 z-10">
                    <div className="flex items-center rounded-md bg-gray-600 px-3">
                      ...
                      <Input placeholder="Search categories..." className="h-9 bg-transparent border-none shadow-none focus-visible:ring-0 focus-visible:ring-offset-0" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
                  </div>
                  <div className={cn("max-h-[200px] overflow-y-auto")}>
                    {filteredCategories.map(cat => <SelectItem key={cat.name} value={cat.name}>
                        {cat.name}
                      </SelectItem>)}
                  </div>
                </SelectContent>
